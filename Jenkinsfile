...
node('master') {
  currentBuild.displayName = "${BUILD_NUMBER}-CicdSelfService"
  ...
  wrap([$class: 'BuildUser']) {
    ...
    stage('Unit Testing') {
       sh """
        ./gradlew clean test
        #hygieiaTestPublishStep buildStatus: 'Success', testApplicationName: 'CicdSelfService', testEnvironmentName: 'CI', testFileNamePattern: '*.html', testResultsDirectory: '/src/cheersApp/build/reports/tests/test/', testType: 'Unit'
       """
    }
      ...
      withCredentials([
        ...
        dir("${env.WORKSPACE}") {
          sh """
             ...
             docker tag demouser/CicdSelfService:latest nexus.gitook.com:8447/demouser/CicdSelfService:${IMAGE_TAG}
             docker push nexus.gitook.com:8447/demouser/CicdSelfService:${IMAGE_TAG}
             docker rmi demouser/CicdSelfService:latest
          """
        }
      }
    }
   ...
   stage ('Deploy to FT Environment in Openshift') {
      ...
      withCredentials([
        ...
        sh """
          ...
          ${oc} tag --source=docker ${nexusRegistry}/CicdSelfService:${IMAGE_TAG} ${env.BUILD_USER_ID}/CicdSelfService-is:latest --insecure
          sleep 5
          ${oc} import-image CicdSelfService-is --confirm --insecure | grep -i "successfully"

          echo "Liveness check URL: http://`${oc} get route CicdSelfService-rt -n ${env.BUILD_USER_ID} -o jsonpath='{ .spec.host }'`/myservice"
        """
      }
    }
...
*/
    ...
    stage('Merge CI Branch to Master') {
       withCredentials([
        ...
        dir("${env.WORKSPACE}") {
          sh """
            git remote set-url origin "https://${env.GIT_USERNAME}:${env.GIT_PASSWORD}@github.com/${env.GIT_USERNAME}/CicdSelfService.git"
